...
export function buildBrowserTools({ defineTool, service, ctx, isDisabled }) {
    ...
    const evaluate = defineTool({
        name: 'browser_eval',
        description: '在内置浏览器的页面上下文执行 JS 表达式，返回 JSON 结果（≤64KB）。' +
    ...
    const screenshot = defineTool({
        ...
        description: '截取内置浏览器当前页面为 PNG：返回落盘路径与尺寸（浏览器面板同步可见）；' +
    ...
    const viewport = defineTool({
        ...
        description: '设置内置浏览器页面视口尺寸（320–3840 × 320–2160，移动端常用 375×812），用于验证响应式/移动布局；' +
