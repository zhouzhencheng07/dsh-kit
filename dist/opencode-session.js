 ...
 */
function blockEnd(lines, start, keyIndent) {
    let j = start;
...
function findHeadersLine(lines, start, end, keyIndent) {
...
function findSessionValue(lines, start, end, parentIndent) {
    for (let i = start; i < end; i += 1) {
        if (indentOf(lines[i].text) <= parentIndent)
            continue;
...
export function findOpenCodeSessionValue(text) {
 ...
 */
export function insertOpenCodeSessionHeader(text, uuid) {
    const existing = findOpenCodeSessionValue(text);
    if (existing)
        return { action: 'exists', text, value: existing };
    const lines = splitLines(text);
    const provider = findProviderLine(lines);
    if (!provider)
        return { action: 'no-provider', text, value: null };
    ...
    const insertAfter = (anchorIndex, inserts) => {
    ...
    if (provider.inline === '{}')
        lines[provider.index].text = `${indentUnit(lines[provider.index].text)}opencode-go:`;
    const end = blockEnd(lines, provider.index + 1, provider.indent);
    const headers = findHeadersLine(lines, provider.index + 1, end, provider.indent);
    if (headers) {
        ...
        if (headers.inline === '{}')
            lines[headers.index].text = `${indentUnit(lines[headers.index].text)}headers:`;
        const headersEnd = blockEnd(lines, headers.index + 1, headers.indent);
...
export function settingsYamlFile() {
...
export function applyOpenCodeSession(ctx) {
        ...
        webCtx.effect(() => {
            const dispose = webCtx.webServer.register({
                ...
                handler: (req, res) => {
                    ...
                    void ensureOpenCodeSession().then((result) => {
                        if (result.action === 'error')
                            json(500, { ok: false, error: result.message });
                        else if (result.action === 'no-provider') {
                            json(404, { ok: false, error: 'settings.yaml 里没有 llm-pi-ai.providers.opencode-go 配置段，无法写入' });
                        }
                        else
                            json(200, { ok: true, action: result.action, value: result.value });
                    }, (error) => {
