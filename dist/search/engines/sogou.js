...
export const sogouEngine = {
    ...
    async search(query, { maxResults = 5, signal } = {}) {
        ...
        const resolved = await Promise.all(blocks.slice(0, maxResults).map(async (b) => {
            if (!b.redirect)
                return { ...b, url: '' };
            try {
...
function parseResultBlocks(html) {
    ...
    for (const wrap of wraps) {
        const titleMatch = wrap.match(/<h3[^>]*class="[^"]*vr-title[^"]*"[^>]*>\s*<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/);
        if (!titleMatch)
            continue;
        const redirect = titleMatch[1] ?? '';
        const title = cleanText(titleMatch[2] ?? '');
        const snippetMatch = wrap.match(/<div[^>]*class="[^"]*fz-mid space-txt[^"]*"[^>]*>([\s\S]*?)<\/div>/);
