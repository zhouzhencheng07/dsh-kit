...
export function vaultSkillMarkdown(root) {
    return [
        ...
        '  写页规则：只记唯一事实、不写流水账（不写日期、不写谁定的、不记过程）。',
        '---',
        '',
        '# 知识库（vault）使用规则',
        '',
        ...
        '## 页面规则',
        '',
        '- **只记唯一事实**：一条事实只写一次——总览/索引页只留一行入口 + 链接，细节归功能页，',
        '  别在两处各写一遍；写前先 `vault_search` 查重，已有同类页就合并/追加，不另开新页。',
        '- **不写流水账**：不写日期、不写「谁定的 / 谁反馈的 / 第几轮改的」、不写实施过程与验证',
        '  清单（那些在 git 提交历史里）。只写「现在是什么 + 为什么这样 + 什么条件下它不再成立」；',
        '  已经不做/退役的直接写成「不做 X（原因）」，不叙述它曾经长什么样。',
        '- **判据**：删掉某句后「反着做会不会坏」——会坏才留；删了不影响后来人做决定的句子不要写。',
...
export async function removeVaultSkill(skillsDir) {
    const dir = path.join(skillsDir, VAULT_SKILL_DIRNAME);
