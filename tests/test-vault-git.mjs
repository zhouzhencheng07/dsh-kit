...
test('存量库升级：本模块写的 .gitignore 缺失忽略项时补齐并提交，幂等', async () => {
  const root = tmp()
  // 复刻存量库形态：模块写过的 .gitignore 缺 *.tmp
  ...
  await ensureVaultGit(root)
  assert.equal(fs.readFileSync(path.join(root, '.gitignore'), 'utf8'), 'attachments/\n*.tmp\n')
  assert.match(git(root, 'log', '--format=%s').trim(), /dsh-kit: 存档忽略项补齐/)
  // 幂等：第二次无事发生（不产生新提交）
...
test('commitVault：无变化跳过，有改动提交，attachments / *.tmp 忽略', async () => {
  const root = tmp()
  fs.writeFileSync(path.join(root, 'a.md'), '# A\n', 'utf8')
  await ensureVaultGit(root)
  // 无变化 → false 且不加提交
