...
- **文件树**（工具行开关）：以会话工作区为根浏览文件，支持新建/重命名/删除
  （回收站）/复制路径；点文件在中间舞台的**「文件」区开一个文档签**（一文件一签、
  可多开，点签切换、✕ 单关；超过设置卡的「文件标签数上限」，默认 8，自动关掉最久
  没看的那个），md 所见即所得，自动保存 + mtime CAS 防覆盖，AI 改盘自动跟随 /
  diff 着色
...
- **知识库**（工具行开关，默认 **Ctrl+Alt+K**）：默认地址开箱即用（数据目录下 `dsh-kit\vault`，可在设置改绝对路径）——选库（顶层目录）
  进入阅读，懒加载目录树，**舞台「知识库」区里每页一个文档签（可多开、✕ 单关）**，`[[双链]]` 页内跳转（前进/后退）+ 反链 + 碎链点击
...
- **工作台三段式布局**（2026-09-10 定稿）：左侧栏放索引（官方会话 ↔ 文件树 ↔
  源代码管理 ↔ 知识库目录 ↔ 日程待办单槽轮换）、中间舞台放当前工作对象、右侧对话列
  常驻（保底 400px）。舞台标签**分两层，和内置浏览器一个形态**——顶层是功能签：文件/
  知识库/后台任务/日程/浏览器各一张（✕ 关整片），属于它的文档签（每个文件、每个
  知识库页）显示在该功能区自己的内容区顶部（点击切换、✕ 单关，超宽横向滚动），
  所以开多少文件都不会把顶栏撑乱；「+」菜单在顶栏右侧。工具行五钮：文件树/源代码
  管理/知识库/日程/终端（**选中态跟侧栏视图走**——侧栏正显示哪个索引就点亮哪个钮，
  舞台里开着什么标签不影响，且四者严格互斥）；侧栏底部按钮区：后台任务/浏览器/计时
  三钮常驻（后台任务带运行中计数角标）；知识库/日程两钮开合成对（侧栏索引与舞台
  功能签一起开、一起关），计时钮起表/运行态/停表确认。宽度模型：拖未钉标签调全局
  共享默认（未钉标签联动），图钉按功能类型钉住当前宽度（localStorage）。**舞台不可
  收起**（同日定稿：隐藏态与快捷键一并取消——舞台是中段，没有「开着但看不见」的
  第三态）；文件签被动打开——文件树/源代码管理/对话链接点开即开
- **后台任务**（舞台「+」菜单或底部钮打开）：列出会话运行中的后台任务，可查看输出、
  结束任务（等同官方 `job_output`/`job_kill`）；顶层签与菜单项带运行中计数
...
`dsh web`：工具行出现文件树/源代码管理/知识库/日程/终端五个开关（选中态跟侧栏
视图走），侧栏底部按钮区承载后台任务/浏览器/计时，中间舞台承载文件标签、知识库
页标签与后台任务/日程/浏览器页（舞台不可收起），AI 的 `web_search` 同时切到免费
多源搜索。
...
- `client/bundle.js`：浏览器半边（手写 ModuleLoader bundle，无构建）——
  ...
  文件树、源代码管理、知识库目录与日程待办共用侧栏槽，中间舞台（顶层功能签 +
  各功能自己的文档签条）、终端坞与计时悬浮窗自绘（CSS 让位）；设置页与设置卡
  注册进 settings 槽位
