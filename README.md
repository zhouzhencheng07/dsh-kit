...
- **文件树**（工具行开关）：以会话工作区为根浏览文件，支持新建/重命名/删除
  （回收站）/复制路径；点文件开舞台可编辑标签（md 所见即所得，自动保存 +
...
- **日程**（侧栏待办索引 + 舞台标签）：侧栏待办列表勾选/计时/点开舞台编辑，
  舞台周时间网格 + 本周统计；agent 经 `schedule_query`/`schedule_create` 只读
  汇总、只建不改（人主导）；数据结构化落盘 `$DSH_HOME/dsh-kit/schedule.json`
- **知识库**（右侧边面板标签）：默认地址开箱即用（数据目录下 `dsh-kit\vault`，可在设置改绝对路径）——选库（顶层目录）
  进入阅读，懒加载目录树，`[[双链]]` 页内跳转（前进/后退）+ 反链 + 碎链点击
...
- **工作台三段式布局**（2026-09-10 定稿）：左侧栏放索引（官方会话 ↔ 文件树 ↔
  ...
  知识库/浏览器标签共存 + 「+」菜单）、右侧对话列常驻（保底 400px）。侧栏底部
  按钮区：知识库/日程/任务/浏览器/计时五钮常驻 + 文件钮被动出现；计时钮起表/
  运行态/停表确认。宽度模型：拖未钉标签调全局共享默认（未钉标签联动），图钉按
  功能类型钉住当前宽度（localStorage）。文件标签被动打开——文件树/源代码管理/
  对话链接点开即开（可编辑）
...
`dsh web`：工具行出现文件树/源代码管理/终端等开关，侧栏底部按钮区 + 中间舞台承载知识库/日程/任务/浏览器等面板，AI 的 `web_search`
同时切到免费多源搜索。
