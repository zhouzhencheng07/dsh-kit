...
工作台住在**官方右侧边栏**（官方 `sidebar.right` 服务）：diff / 知识库 / 日程 /
...
- **源代码管理**（工具行开关 / 默认 **Ctrl+Alt+.**）：页内 git 工作台——暂存/取消暂存/
  ...
  非 git 目录可一键初始化
- **知识库 · 日程**（一个组件行；行开关 = 总开关，关掉即目录索引、两张 dock 签、输入行
  入口与 4 个日程工具一起退场）
  - **知识库**（工具行开关 / 默认 **Ctrl+Alt+/**）：默认地址开箱即用（数据目录下
  `dsh-kit\vault`，可在本行配置页改绝对路径）——左栏一行式搜索 + 目录树选页，右栏「知识库」
...
- **快捷键**：四条命令（终端 / 文件树 / 源代码管理 / 知识库）注册进**宿主「快捷键」页**
  ...
  （web 左栏 Ctrl+Alt+B、右栏 Ctrl+Shift+B）。改键去官方页，插件配置页不管键位。
  插件里**悬停提示统一是官方气泡**
...
**宿主版本要求**：dsh ≥ 0.1.7-rc.2（组件行与插件页行配置、官方 `shortcuts` 服务、
