...
- **知识库**（工具行开关 / **Ctrl+Alt+K**，默认关）：默认地址开箱即用（数据目录下
  `dsh-kit\vault`，可在设置改绝对路径）——**纯只读阅读面**：左栏目录树选页，右栏
  「知识库」签里每页一个文档签（可多开、✕ 单关），`[[双链]]` 页内跳转（前进/后退，
  `[[页#标题]]` 落到对应小节）+ 阅读条「目录/反链」+ 全文搜索（文件名优先打分）；
  与对话互通——聊天里的 vault 路径点击直达、树上「@」把页面/选区插进输入框；
  库就是普通 md 目录，插件只读不写（不建目录、不碰 git），写入交给 agent 文件工具或你的本地编辑器
...
- **余额与用量**（默认关）：composer 下方状态带最右端显示一张芯片，跟随当前会话选中的模型
  ...
  提示限流风险（DeepSeek：工作日 9:00–12:00、14:00–18:00；GLM：工作日 14:00–18:00）；
