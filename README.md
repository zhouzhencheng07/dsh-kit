...
- **日程**（右侧边面板标签）：待办列表 + 本周统计在上，周时间网格在下；待办可
  计时（计时芯片在待办卡头）；agent 经 `schedule_query`/`schedule_create` 只读
  汇总、只建不改（人主导）；数据结构化落盘 `$DSH_HOME/dsh-kit/schedule.json`
- **知识库**（右侧边面板标签）：默认地址开箱即用（数据目录下 `dsh-kit\knowledge`，可在设置改绝对路径）——选库（顶层目录）
  进入阅读，懒加载目录树，`[[双链]]` 页内跳转（前进/后退）+ 反链 + 碎链点击
  建页 + 全文搜索；CodeMirror 源码编辑（mtime CAS 防覆盖）；根目录自动生成
  AGENTS.md 约定，agent 文件工具直接读写（文件即接口）；git 自动存档——
  初始化一次 + 人工保存/AI 编辑前/删除后各一提交，随时整体回退（未装 git 自动跳过，
  attachments/ 不入存档）
- **右侧边面板**（常置）：文件预览 / 后台任务 / 日程 / 内置浏览器四标签共存切换，
  标签旁「+」随时开标签，全部关掉后显示空态选择器；最小化收成右缘竖条（刷新保持）；
