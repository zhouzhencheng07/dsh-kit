...
- **知识库**（工具行开关 / **Ctrl+Alt+K**，默认关）：默认地址开箱即用（数据目录下
  ...
  库就是普通 md 目录，agent 直接用文件工具读写；git 自动存档（初始化一次 + 人工保存/删除后各一提交）
...
- **会话通知**（默认开）：回合收尾、或 agent 提问 / 等你批准工具调用 / 提交计划待批时弹桌面通知
  （浏览器 Notification API，点击通知回到对应会话）——页面切到后台标签/别的窗口、
  或事件发生在你没打开的那个会话时提醒；人正看着该会话时不打扰。限流自动续跑期间
  不会误报「完成」（真的停下时才提醒），权限在设置卡里一键申请（未授权时退化为
  标签标题上的未读计数）
