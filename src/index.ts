...
import { teeRegistryJobs, panelReadJobOutput } from './job-tee.ts'
    ...
    webCtx.effect(() => {
              ...
              runGit(['rev-list', '--parents', '-n', '1', full], root).then((pl) => {
                ...
                runGit(['-c', 'core.quotePath=false', 'diff-tree', '--root', '-p', '--no-commit-id', full, '--', rel], root).then((d) => {
                  ...
                  runGit(['-c', 'core.quotePath=false', 'show', `${full}:${rel.replace(/\\/g, '/')}`], root).then((blob) => {
                    const text =
                      blob.ok && typeof blob.out === 'string' && blob.out.length > 0 && blob.out.length <= 1024 * 1024
                        ? blob.out
                        : null
                    if (text !== null) {
      ...
      const disposeGitLog = webCtx.webServer.register({
        ...
        handler: (req, res) => {
          ...
          if (skip > 100000) skip = 100000
          runGit(
            ['-c', 'core.quotePath=false', 'log', 'HEAD', '--branches', '--tags', '--remotes', '--topo-order',
              `--skip=${skip}`, `--max-count=${n + 1}`,
              '--pretty=format:%H%x1f%P%x1f%h%x1f%an%x1f%at%x1f%s%x1f%D%x1e'],
            root,
          ).then((r) => {
            if (!r.ok) {
      ...
      ctx.inject(['workspaceRegistry', 'sessionQuery'], (regCtx: { workspaceRegistry: any; sessionQuery: any }) => {
        ...
        webCtx.webServer.register({
          ...
          handler: (_req, res) => {
            void (async () => {
              const ids = (workspaceRegistry.archivedSessionIds as string[]).map(String)
              ...
              try {
                for (const r of await sessionQuery.readTitleSnapshots(ids)) {
                  ...
                  if (snap && typeof snap.title === 'string' && snap.title !== '') folded.set(r.value.session.id, snap.title)
                }
              } catch {
                /* 标题折叠失败按无标题处理 */
              }
