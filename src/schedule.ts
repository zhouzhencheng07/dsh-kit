...
export class ScheduleStore {
  ...
  stats(scope: 'day' | 'week' | 'month', date: string, now?: Date): { timedMs: number; totalMs: number; eventCount: number; completedCount: number; openCount: number } {
    ...
    for (const o of expandOccurrences(this.data.events, from, to)) {
      if (o.allDay || withEntries.has(o.baseId)) continue
      const end = o.endMins ?? o.startMins + 60
      const passed = o.date < nowDate || (o.date === nowDate && end <= nowMins)
      if (passed) elapsedMs += Math.max(0, end - o.startMins) * 60000
    }
...
function createSummary(ev: ScheduleEvent): string {
  const kind = ev.start !== undefined ? '日程' : '待办'
  const bits: string[] = []
  if (ev.allDay === true) bits.push(`${ev.start?.slice(0, 10) ?? ''} 全天`)
...
export function toolArgsToCreateInput(args: Record<string, unknown>): Record<string, unknown> {
  ...
  if (args.allDay === true) {
    ...
    if (endTime !== null) input.end = `${date}T${endTime}`
  } else {
    input.due = date
  }
  const repeat = args.repeat
  if (repeat === 'daily' || repeat === 'weekly' || repeat === 'monthly') {
    if (!('start' in input)) throw new Error('repeat 仅对日程生效：请提供 time 或 allDay=true')
...
export function buildScheduleTools({ defineTool, store }: { defineTool: DefineTool; store: ScheduleStore }): ToolDefinition[] {
  ...
  const create = defineTool({
    ...
    description:
      '在用户日程里创建条目。date+time 或 allDay 创建日程事件（周网格显示），只给 date 创建待办（待办列表显示）。' +
