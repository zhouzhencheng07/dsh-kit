...
/**
 * sanitizeFields 的输出。可清空字段以 null 表达「显式清空」（三态：值 = 落值、
 * null = 清、缺省 = 不动）；create 只认值，null 在 create 里等价于不带。
 */
...
function sanitizeFields(input: Record<string, unknown>, patch: boolean): SanitizedFields {
  ...
  const clearableText = (key: 'description' | 'location', cap: number): void => {
    if (!has(key)) return
    const v = input[key]
    if (typeof v === 'string') out[key] = v === '' ? null : v.slice(0, cap)
    else if (v === null && patch) out[key] = null
  }
  clearableText('description', 2000)
  clearableText('location', 200)
  const clearableDT = (key: 'start' | 'end'): void => {
    if (!has(key)) return
    const v = input[key]
    if (typeof v === 'string' && DT_RE.test(v)) out[key] = v.slice(0, 16)
    else if (v === null && patch) out[key] = null
  }
  clearableDT('start')
  clearableDT('end')
  ...
  if (has('due')) {
    ...
    if (d !== undefined) out.due = d
    else if (input.due === null && patch) out.due = null
  }
  ...
  if (has('skip')) {
    const v = input.skip
    if (Array.isArray(v)) {
      const days = [...new Set(v.filter((s): s is string => typeof s === 'string' && DATE_RE.test(s)))].sort()
  ...
  if (has('completedAt')) {
    ...
    if (typeof v === 'string' && (DATE_RE.test(v) || DT_RE.test(v))) out.completedAt = v.slice(0, 16)
    else if (v === null && patch) out.completedAt = null
  }
...
function applyFields(ev: ScheduleEvent, f: SanitizedFields): void {
  const setOrClear = <K extends keyof ScheduleEvent>(key: K, value: ScheduleEvent[K] | null | undefined): void => {
    if (value === undefined) return
    if (value === null) delete ev[key]
    else ev[key] = value
  }
  if (f.title !== undefined) ev.title = f.title
  setOrClear('description', f.description)
  setOrClear('location', f.location)
  setOrClear('start', f.start)
  setOrClear('end', f.end)
  if (f.recurrence !== undefined) ev.recurrence = f.recurrence
  if (f.color !== undefined) ev.color = f.color
  setOrClear('due', f.due)
  setOrClear('skip', f.skip)
  setOrClear('completedAt', f.completedAt)
  if (f.parentId !== undefined) ev.parentId = f.parentId
}

/** 待办是否已逾期：纯日期到**当天结束前**都不算逾期（只比"今天"），带时刻比到分钟 */
function dueOverdue(ev: ScheduleEvent, now: Date): boolean {
  if (ev.start !== undefined || ev.completedAt != null || ev.due === undefined) return false
  if (ev.due.includes('T')) return ev.due.slice(0, 16) <= dtStrOf(now)
 ...
 */
function validateScheduleEvent(ev: ScheduleEvent): void {
  if (ev.start !== undefined) {
    if (ev.end === undefined) throw new Error('日程必须有结束时刻（end 与 start 成对）')
    const s = parseDT(ev.start)
    const e = parseDT(ev.end)
    if (!s || !e) throw new Error('start/end 无法解析')
    if (e.getTime() <= s.getTime()) throw new Error('end 必须晚于 start')
    delete ev.due
...
export class ScheduleStore {
  ...
  update(id: string, patch: Record<string, unknown>): ScheduleEvent | null {
    ...
    const draft: ScheduleEvent = { ...current }
  ...
  stats(scope: 'day' | 'week' | 'month', date: string, now?: Date): { timedMs: number; totalMs: number; eventCount: number; completedCount: number; openCount: number } {
    ...
    let openCount = 0
    for (const o of occ) {
      if (o.state === 'past') completedCount++
      else openCount++
    }
    ...
    for (const o of occ) {
      ...
      if (withEntries.has(o.baseId)) continue
  ...
  summary(scope: 'day' | 'week' | 'month', date: string): string {
    ...
    // 到当天结束前不算逾期，带时刻比到分钟）
    const now = new Date()
    const isOverdue = (ev: ScheduleEvent): boolean => dueOverdue(ev, now)
    const dueDayOf = (ev: ScheduleEvent): string => ev.due?.slice(0, 10) ?? ''
    ...
    const doneCount = this.data.events.filter(
      (e) => e.start === undefined && e.completedAt != null && e.completedAt.slice(0, 10) >= from && e.completedAt.slice(0, 10) <= to,
    ).length
...
export function dtStrOf(d: Date): string {
...
function occDurationMins(o: Occurrence): number {
...
function occurrenceState(
  ...
  if (nowDate > endDay || (nowDate === endDay && nowMins >= end)) return 'past'
  if (nowDate < date || (nowDate === date && nowMins < startMins)) return 'todo'
  return 'doing'
}
 ...
 */
export function expandOccurrences(events: ScheduleEvent[], from: string, to: string, now: Date = new Date()): Occurrence[] {
  ...
  for (const ev of events) {
    ...
    const mk = (date: string, virtual: boolean): Occurrence => {
    ...
    for (let d = iterFrom; d <= iterTo; d = addDays(d, 1)) {
      ...
      if (!hit) continue
      const occ = mk(d, true)
      if (occ.date > to || occ.endDate < from) continue
      out.push(occ)
    }
  }
...
const REC_TYPE_ZH = { daily: '每天', weekly: '每周', monthly: '每月' } as const

function recurrenceLabel(rec: ScheduleRecurrence): string {
  ...
  if (rec.days !== undefined && rec.days.length > 0) bits.push(`(${rec.days.join(',')})`)
  if (rec.end !== undefined) bits.push(`至${rec.end}`)
  return bits.join('')
}

/** 条目一句话摘要：`日程：例会（2026-09-07 09:00–10:00，每周(1)）`——create/update 工具共用 */
function eventDigest(ev: ScheduleEvent): string {
  const kind = ev.start !== undefined ? '日程' : '待办'
  const bits: string[] = []
  if (ev.start !== undefined) {
    bits.push(`${ev.start.replace('T', ' ')}${ev.end !== undefined ? `–${ev.end.split('T')[1] ?? ''}` : ''}`)
  } else if (ev.due !== undefined) bits.push(`截止 ${ev.due}`)
  if (ev.recurrence) bits.push(recurrenceLabel(ev.recurrence))
 ...
 * 会变成永不展开的死配置，故直接拒绝。
 */
export function toolArgsToCreateInput(args: Record<string, unknown>): Record<string, unknown> {
  ...
  if (date === '' && DT_RE.test(rawDate)) {
    ...
    if (dtTime === null) throw new Error(`date 无法解析：${args.date as string}`)
    date = rawDate.slice(0, 10)
  }
  if (date === '') throw new Error('date 必填，格式 YYYY-MM-DD')
  const timeArg = typeof args.time === 'string' && args.time.trim() !== '' ? args.time : null
  const time = timeArg !== null ? normHHmm(timeArg) : dtTime
  if (timeArg !== null && time === null) throw new Error(`time 无法解析：${timeArg}`)
  ...
  if (endTimeArg !== null && endTime === null) throw new Error(`endTime 无法解析：${endTimeArg}`)
  ...
  if (time !== null) {
    ...
    if (end === null) {
      ...
      if (plusHour >= 1440) {
        ...
        if (endDate === null) endDate = addDays(date, 1)
      } else {
    ...
    if (endDT <= start) throw new Error(`end 必须晚于 start：start=${start}、end=${endDT}`)
    input.start = start
    input.end = endDT
  } else {
    input.due = date
...
export function buildScheduleTools({
  ...
  const query = defineTool({
    name: 'schedule_query',
    description:
      '查询用户的日程汇总（日/周/月粒度）：带时刻的事件、逾期待办、即将到期与已完成的待办、累计计时。' +
  ...
  const create = defineTool({
    ...
    description:
      '在用户日程里创建条目。三种用法：① 只给 date（YYYY-MM-DD）= 待办（待办列表显示）；' +
  ...
  const upd = defineTool({
    ...
    description:
      '修改已有日程条目（部分更新：只给要改的字段，其余不动；id 来自 schedule_query）。' +
