...
export function recycleDeleteBatch(targets: string[]): Promise<boolean[]> {
  return new Promise((resolve) => {
    ...
    const script =
      `try{Add-Type -AssemblyName Microsoft.VisualBasic}catch{exit 1};` +
      `$i=0;` +
      `foreach($p in @(${list})){` +
      `try{` +
    ...
    child.on('close', () => {
      ...
      for (const m of out.matchAll(/R(\d+)=(1|0)/g)) {
        ...
        if (idx >= 0 && idx < results.length) results[idx] = m[2] === '1'
      }
      finish(results)
    })
  })
...
export function recycleDelete(target: string): Promise<boolean> {
