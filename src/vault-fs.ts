...
export function parseConflict(raw: unknown): Conflict {
...
export function safeLeaf(raw: unknown): string | null {
  ...
  if (/[\u0000-\u001f<>:"|?*]/.test(name)) return null
  if (WIN_RESERVED_NAME.test(name.split('.')[0] ?? '')) return null
  if (name.length > 120) return null
  return name
}

/** 多级名字（新建用）：逐段过 safeLeaf，空段丢弃；全空返 '' */
export function safeRel(raw: unknown, maxSegs = 8): string {
...
export function dedupeName(dir: string, name: string, exists: (p: string) => boolean = existsSync): string | null {
...
export function resolveInside(root: string, raw: string, opts: { allowRoot?: boolean } = {}): string {
  const trimmed = String(raw ?? '').trim()
  if (trimmed === '') throw new Error('缺少路径')
  // `..` 段必须在**归一前**判：path.resolve 会把它解掉，之后再看已无从发现
  for (const seg of trimmed.split(/[/\\]+/)) {
    if (seg === '..') throw new Error(`路径含上跳段：${trimmed}`)
  }
...
export function inLibrary(root: string, target: string): boolean {
...
export function isLibraryRoot(root: string, target: string): boolean {
...
export function resolveWikiRel(pages: WikiRef[], target: string, hostSpace: string): string | null {
  const t = stripMd(String(target ?? '').trim().replace(/\\/g, '/')).toLowerCase()
  if (t === '') return null
  const base = t.split('/').pop() ?? t
  const pref = hostSpace === '' ? null : `${hostSpace.toLowerCase()}/`
  const better = (cand: WikiRef, cur: WikiRef | null): boolean => {
    if (cur === null) return true
    if (pref === null) return false
    return cand.rel.toLowerCase().startsWith(pref) && !cur.rel.toLowerCase().startsWith(pref)
  }
  let exact: WikiRef | null = null
  let suffix: WikiRef | null = null
  let byName: WikiRef | null = null
  for (const p of pages) {
    const relLower = p.rel.toLowerCase()
    if (relLower === t && better(p, exact)) exact = p
    if (t.includes('/') && relLower.endsWith(`/${t}`) && better(p, suffix)) suffix = p
    if ((relLower.split('/').pop() ?? relLower) === base && better(p, byName)) byName = p
  }
  return (exact ?? suffix ?? byName)?.rel ?? null
}

/** rel 的最后 n 段（n 超过段数就整段）——移动改写保持原写法的"段数形状" */
export function tailSegments(rel: string, n: number): string {
...
export type RewriteMode = { kind: 'name'; name: string } | { kind: 'move'; rel: string }

/** `s` 以 `[[` 开头：解析到 oldRel 才改写，返回（消费的字符数, 改写后的整段）。
 *  锚点 `#锚` 与别名 `|别名`、目标两侧空白原样保留 */
function rewriteOneLink(
  s: string,
  hostSpace: string,
  pages: WikiRef[],
  oldRel: string,
  ...
  if (resolveWikiRel(pages, target, hostSpace) !== oldRel) return null
  let next: string
  if (mode.kind === 'name') next = mode.name
  else {
...
export function rewriteWikiLinks(
  ...
  if (mode.kind === 'name' && mode.name === '') return content
  let out = ''
  let fence: string | null = null
  let inline: number | null = null // 行内代码的反引号长度（null = 不在代码里）
...
async function settleConflict(dir: string, name: string, conflict: Conflict): Promise<{ name: string; skipped: boolean }> {
  if (!existsSync(path.join(dir, name))) return { name, skipped: false }
  if (conflict === 'skip') return { name, skipped: true }
  if (conflict === 'overwrite') {
    await discard(path.join(dir, name))
    return { name, skipped: false }
  }
  const next = dedupeName(dir, name)
  if (next === null) throw new Error('同名文件太多，无法自动编号')
...
export function createEntry(
  ...
  if (rel === '') throw new Error('名称非法或为空')
  const segs = rel.split('/')
  ...
  if (kind === 'dir') {
    ...
    if (existsSync(target)) return { path: target, exists: true }
    fs.mkdirSync(target)
    return { path: target }
  }
  fs.mkdirSync(parent, { recursive: true })
...
export function renameEntry(
  ...
  rawName: unknown,
  pages: WikiRef[] = [],
): { path: string; links: number } {
  const target = resolveInside(root, targetAbs)
  if (isLibraryRoot(root, target)) throw new Error('资料库根目录不能改名')
  const leaf = safeLeaf(rawName)
  if (leaf === null) throw new Error('名称非法或为空')
  ...
  if (!inLibrary(root, target) && !/\.(md|markdown)$/i.test(target)) throw new Error('只允许重命名笔记页、资料或文件夹')
...
export async function moveEntry(
  ...
  conflict: Conflict,
  pages: WikiRef[] = [],
): Promise<{ path: string; rel: string; name: string; links: number; skipped: boolean }> {
  const target = resolveInside(root, targetAbs)
  const dest = resolveInside(root, destAbs, { allowRoot: true })
  if (isLibraryRoot(root, target)) throw new Error('资料库根目录不能移动')
  if (!fs.statSync(dest).isDirectory()) throw new Error('目标不是文件夹')
  const stat = fs.statSync(target)
...
function rewriteRefs(pages: WikiRef[], oldRel: string, mode: RewriteMode, selfPath: string): number {
  let changed = 0
  for (const page of pages) {
    const abs = page.path
    if (typeof abs !== 'string' || abs === selfPath) continue
    if (!page.links?.some((l) => resolveWikiRel(pages, l, page.space) === oldRel)) continue
    let text: string
    try {
...
function attachmentPath(root: string, bytes: Buffer, ext: string): string {
  const hash = crypto.createHash('sha256').update(bytes).digest('hex')
  return path.join(root, 'attachments', hash.slice(0, 2), `${hash.slice(0, 16)}${ext}`)
}

/** 正文里的本地图片引用（相对路径才收）：`![](src "title")` 与 `<img src="...">` */
function scanImageSrcs(text: string): Array<{ start: number; end: number; src: string }> {
  const out: Array<{ start: number; end: number; src: string }> = []
  const md = /!\[[^\]]*\]\(\s*(<[^>]*>|[^()\s]+)(?:\s+["'][^"']*["'])?\s*\)/g
...
function isLocalImageRef(src: string): boolean {
  ...
  if (/^attachments[\\/]/i.test(s)) return false
  return true
}

/** 收页面引用的本地图片进 attachments/ 并改写引用；返回 [新正文, 新落盘张数] */
export function pullImages(text: string, srcDir: string, root: string): [string, number] {
  const refs = scanImageSrcs(text)
  if (refs.length === 0) return [text, 0]
  let out = text
  let fresh = 0
  for (const ref of refs.reverse()) {
    if (!isLocalImageRef(ref.src)) continue
    let decoded = ref.src
    try {
...
export async function importEntry(
  root: string,
  opts: ImportOptions,
): Promise<{ path: string; rel: string; name: string; images: number; skipped: boolean; renamed: boolean }> {
  ...
  if (bytes === null) throw new Error('缺少导入内容')
  if (bytes.length === 0) throw new Error('文件是空的')
  ...
  if (asPage) {
    ...
    if (existsSync(path.join(dest, want))) {
      ...
      if (opts.conflict === 'overwrite') await discard(path.join(dest, want))
      else {
        const deduped = dedupeName(dest, want)
        if (deduped === null) throw new Error('同名文件太多，无法自动编号')
        final = deduped
      }
    }
  ...
  if (!library && !/\.(md|markdown)$/i.test(fileLeaf)) throw new Error('笔记区只收 md 文件（文献请导进资料库）')
...
export async function deleteEntries(root: string, paths: unknown[]): Promise<{ deleted: number; failed?: string[] }> {
  if (!Array.isArray(paths) || paths.length === 0) throw new Error('缺少要删除的路径')
  const targets: string[] = []
  const seen = new Set<string>()
  for (const raw of paths) {
    const target = resolveInside(root, String(raw ?? ''))
    if (isLibraryRoot(root, target)) throw new Error('资料库根目录不能删除')
